...
export async function weatherReport(lat, lon) {
  try {
    ...
    if (!report.ok) {
      throw new Error(`Weather API error: ${report.status}`);
    }
    ...
    if (!forecastReturn.ok) {
      throw new Error(`Forecast API error: ${forecastReturn.status}`);
    }
...
export async function getCoordinates(city) {
  try {
    ...
    if (!location.ok) {
      throw new Error(`Nominatim API error: ${location.status}`);
    }
